...
namespace Utility
{
	...
	std::vector<std::string> GetFilesInDirectory(const std::string& dir)
	{
		...
		{
			Utility::Print(L"Unable to find folder at %s", dir.c_str());
			Utility::Break();
			return {};
		}
		...
		if (hFind == INVALID_HANDLE_VALUE)
		{
			Utility::Print("Something went wrong %s", dir.c_str());
		}
	...
	std::string ReadFileToString(const std::string& filePath)
	{
		...
		if (file == INVALID_HANDLE_VALUE)
		{
			Utility::Print("Was unable to open the file: [%s]", filePath.c_str());
			Utility::Break();
			return "";
		}
		...
		{
			Utility::Print("Was unable to read the file: [%s]", filePath.c_str());
			Utility::Break();
		}
		if (readBytes != fileSize)
		{
			Utility::Print("Was unable to read the entire file: [%s]", filePath.c_str());
			Utility::Break();
		}
		CloseHandle(file);
	...
	bool WriteFile(const void* data, size_t size, const std::string& filePath)
	{
		...
		{
			Utility::Print("trying to write more than %llu amount of bytes [%llu], this isn't supported");
			Utility::Break();
		}
		if (::WriteFile(file, data, static_cast<DWORD>(size), &bytesWritten, NULL) == false)
		{
			Utility::Print("Was unable to write the file: [%s]", filePath.c_str());
			Utility::Break();
			CloseHandle(file);
			return false;
		}
		...
		{
			Utility::Print("Was unable to write the entire file: [%s]", filePath.c_str());
			Utility::Break();
			CloseHandle(file);
			return false;
		}
