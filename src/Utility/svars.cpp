...
namespace Utility
{
	...
	int SVars::GetSVarInt(SVar svar)
	{
		...
		Utility::Print("Attempting to get svar: %s as integer!", GetStringSVar(svar));
		return 0;
	}
	...
	float SVars::GetSVarFloat(SVar svar)
	{
		...
		Utility::Print("Attempting to get svar: %s as float!", GetStringSVar(svar));
		return 0.0f;
	}
	...
	void SVars::SetSVarInt(SVar svar, int val)
	{
		...
		if (it != m_SVars.end())
		{
			...
			{
				Utility::Print("Attempting to set svar: %s as int!", GetStringSVar(svar));
			}
		}
		else
		{
	...
	void SVars::SetSVarFloat(SVar svar, float val)
	{
		...
		if (it != m_SVars.end())
		{
			...
			{
				Utility::Print("Attempting to set svar: %s as float!", GetStringSVar(svar));
			}
		}
		else
		{
